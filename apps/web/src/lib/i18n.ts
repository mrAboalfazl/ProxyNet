export type Lang = 'en' | 'fa';

const tr: Record<Lang, Record<string, string>> = {
  en: {
    brand: 'ProxyNet',
    signin: 'Sign in',
    signout: 'Sign out',
    register: 'Register',
    save: 'Save',
    cancel: 'Cancel',
    loading: 'Loading…',
    back_home: 'Home',

    // Language toggle
    'lang.en': 'EN',
    'lang.fa': 'FA',

    // Landing nav
    'nav.signin': 'Sign in',
    'nav.getstarted': 'Get started',

    // Landing hero
    'hero.badge': 'Network online · 1 active node',
    'hero.title': 'A secure, reliable network for modern applications',
    'hero.subtitle':
      'Secure connectivity, flexible routing through managed infrastructure, and clear usage controls for teams and individuals.',
    'hero.cta_primary': 'Create free account',
    'hero.cta_secondary': 'Sign in',

    // Landing features
    'features.title': 'Everything you need',
    'features.subtitle':
      'Purpose-built for privacy, performance, reliability, and transparent network management.',
    'feat.0.title': 'Secure Connections',
    'feat.0.desc':
      'Strong encryption and privacy-focused design for dependable connections.',
    'feat.1.title': 'Global Exit Nodes',
    'feat.1.desc':
      'Route through verified exit nodes across multiple countries. Choose a specific country or let the platform pick the optimal path.',
    'feat.2.title': 'Zero-Overhead Forwarding',
    'feat.2.desc':
      'Efficient routing helps deliver responsive performance while keeping connections protected.',
    'feat.3.title': 'Reliable Connectivity',
    'feat.3.desc':
      'Health-aware routing and adaptable endpoints help maintain stable service as network conditions change.',
    'feat.4.title': 'Usage Monitoring',
    'feat.4.desc':
      'Real-time bandwidth and connection counters per credential. Know exactly how your allocation is being used.',
    'feat.5.title': 'Multiple Credentials',
    'feat.5.desc':
      'Generate separate proxy credentials for different devices. Revoke a single credential without affecting others.',

    // Landing steps
    'steps.title': 'Up and running in minutes',
    'steps.subtitle':
      "No configuration headaches. Three steps and you're connected.",
    'step.1.title': 'Create an account',
    'step.1.desc': 'Register with your email and set a password.',
    'step.2.title': 'Generate a credential',
    'step.2.desc': 'Create a connection credential from your dashboard.',
    'step.3.title': 'Connect',
    'step.3.desc':
      'Use the connection details with your preferred compatible client.',

    // Landing CTA
    'cta.title': 'Ready to connect?',
    'cta.subtitle':
      'Create your free account and generate your first credential in under 2 minutes.',
    'cta.btn': 'Get started for free',

    // Landing footer
    'footer.rights': '© 2026 ProxyNet. All rights reserved.',
    'footer.signin': 'Sign in',
    'footer.register': 'Register',

    // Login
    'login.title': 'ProxyNet',
    'login.subtitle': 'Sign in to continue',
    'login.tab.password': 'Password',
    'login.tab.sms': 'SMS OTP',
    'login.email': 'Email',
    'login.email.placeholder': 'you@example.com',
    'login.password': 'Password',
    'login.password.placeholder': '••••••••',
    'login.phone': 'Phone number',
    'login.phone.placeholder': '09xxxxxxxxx',
    'login.sendcode': 'Send code',
    'login.sending': 'Sending…',
    'login.code': 'Verification code',
    'login.code.placeholder': '6-digit code',
    'login.code_sent': 'Code sent! Enter the 6-digit code below.',
    'login.submit': 'Sign in',
    'login.signing': 'Signing in…',
    'login.verify': 'Verify & sign in',
    'login.no_account': 'No account?',
    'login.home': 'Home',
    'login.change_phone': 'Change number',

    // Register
    'reg.title': 'Create account',
    'reg.subtitle': 'Join ProxyNet in seconds',
    'reg.name': 'Display name',
    'reg.name.placeholder': 'Your name',
    'reg.email': 'Email',
    'reg.email.placeholder': 'you@example.com',
    'reg.phone': 'Mobile number',
    'reg.phone.placeholder': '09xxxxxxxxx',
    'reg.phone.hint': 'Required for SMS verification and sign-in',
    'reg.password': 'Password',
    'reg.password.placeholder': 'Min. 6 characters',
    'reg.confirm': 'Confirm password',
    'reg.confirm.placeholder': 'Repeat password',
    'reg.submit': 'Create account',
    'reg.creating': 'Creating account…',
    'reg.have_account': 'Already have an account?',
    'reg.err.mismatch': 'Passwords do not match',
    'reg.err.short': 'Password must be at least 6 characters',

    // Admin layout
    'admin.panel': 'Admin Panel',
    'admin.nav.dashboard': 'Dashboard',
    'admin.nav.statistics': 'Statistics & Budget',
    'admin.nav.nodes': 'Nodes',
    'admin.nav.users': 'Users',
    'admin.nav.plans': 'Plans',
    'admin.nav.pricing': 'Pricing',
    'admin.nav.countries': 'Countries',
    'admin.signout': 'Sign out',

    // User layout
    'user.panel': 'ProxyNet',
    'user.nav.dashboard': 'Dashboard',
    'user.nav.credentials': 'My Credentials',
    'user.nav.profile': 'Profile',
    'user.signout': 'Sign out',

    // User dashboard
    'udash.welcome': 'Welcome back',
    'udash.bandwidth': 'Bandwidth Used',
    'udash.connections': 'Connections',
    'udash.credentials': 'Active Credentials',
    'udash.monthly': 'Monthly Bandwidth',
    'udash.plan': 'Current Plan',
    'udash.of': 'of',
    'udash.used': 'used',
    'udash.period': 'Period',
    'udash.no_plan': 'No active plan',
    'udash.no_plan_msg': 'Contact your administrator to get a plan assigned.',
    'udash.proxy_title': 'Proxy Credentials',
    'udash.proxy_msg':
      'Manage your VLESS credentials and copy connection URIs.',
    'udash.manage': 'Manage credentials',
    'udash.account_title': 'Account Settings',
    'udash.account_msg': 'Update your display name and routing preferences.',
    'udash.edit_profile': 'Edit profile',

    // User credentials
    'cred.title': 'My Credentials',
    'cred.new': 'New credential',
    'cred.create': 'Create',
    'cred.revoke': 'Revoke',
    'cred.copy': 'Copy URI',
    'cred.copied': 'Copied!',
    'cred.empty': 'No credentials yet',
    'cred.empty_msg': 'Create your first proxy credential to start connecting.',
    'cred.label.placeholder': 'Label (optional)',
    'cred.created': 'Created',
    'cred.status.active': 'Active',
    'cred.status.revoked': 'Revoked',

    // User nodes
    'user.nav.nodes': 'My Nodes',
    'user.nav.forwarders': 'Forwarders',
    'user.nav.wallet': 'Wallet',
    'user.nav.billing': 'Usage & billing',

    // Wallet page
    'wallet.title': 'Wallet',
    'wallet.balance': 'Balance',
    'wallet.currency': 'toman',
    'wallet.low_balance_warning':
      'Your balance is below the minimum required to make a call. Top up to continue using the API.',
    'wallet.zero_balance':
      'Your wallet is empty. Every request needs at least {min} toman.',
    'wallet.pricing_title': 'Current pricing',
    'wallet.pricing_per_request': 'per request',
    'wallet.pricing_per_mb': 'per MB SOCKS5 bandwidth',
    'wallet.pricing_min_balance': 'minimum to make a call',
    'wallet.transactions': 'Recent transactions',
    'wallet.no_transactions': 'No transactions yet.',
    'wallet.tx.date': 'Date',
    'wallet.tx.type': 'Type',
    'wallet.tx.description': 'Description',
    'wallet.tx.amount': 'Amount',
    'wallet.tx.balance': 'Balance',
    'wallet.topup_note':
      'To add funds, contact support. Automatic payment integration is coming soon.',
    'wallet.type.topup_admin': 'Admin top-up',
    'wallet.type.forwarder_call': 'Forwarder call',
    'wallet.type.gateway_call': 'Gateway call',
    'wallet.type.adjustment_admin': 'Admin adjustment',
    'wallet.type.refund': 'Refund',

    // Forwarders page
    'fwd.title': 'URL Forwarders',
    'fwd.intro':
      "Create a short URL that routes through our network to any external destination. Perfect for API calls in your code — paste our URL where you'd normally use the target URL.",
    'fwd.new': 'New Forwarder',
    'fwd.empty': 'You have no forwarders yet.',
    'fwd.form.label': 'Label',
    'fwd.form.label_ph': 'e.g., "Weather API" or "GitHub calls"',
    'fwd.form.target': 'Target URL',
    'fwd.form.target_ph': 'https://api.example.com/data',
    'fwd.form.target_help':
      'The destination where forwarded requests will be sent.',
    'fwd.form.forward_auth': 'Forward Authorization header to target',
    'fwd.form.preserve_path': "Append client's path suffix to target URL",
    'fwd.form.preserve_query': 'Forward query string to target',
    'fwd.form.create': 'Create',
    'fwd.your_url': 'Your forwarder URL',
    'fwd.your_url_help':
      'Paste this into your code in place of the target URL. Any HTTP method, headers, body, path suffix, and query string are proxied through.',
    'fwd.calls': 'calls',
    'fwd.last_used': 'last used',
    'fwd.never_used': 'never used',
    'fwd.enabled': 'Enabled',
    'fwd.disabled': 'Disabled',
    'fwd.disable': 'Disable',
    'fwd.enable': 'Enable',
    'fwd.delete': 'Delete',
    'fwd.confirm_delete':
      'Delete this forwarder? The URL will stop working immediately.',
    'fwd.example': 'Example',
    'fwd.test': 'Test',
    'fwd.testing': 'Testing…',
    'fwd.test_ok': 'OK',
    'fwd.test_fail': 'Failed',
    'fwd.test_hint': 'Sends a GET to your forwarder URL. Counts as one call.',

    // Usage examples panel
    'examples.title': 'How to use',
    'examples.intro.forwarder':
      'Copy your forwarder URL into your code — no auth headers, no wrapper. Any method works.',
    'examples.intro.gateway':
      'POST any URL to /gateway/fetch with your UUID + Secret as Basic auth. Response comes back JSON-wrapped so your code can inspect status + headers.',
    'examples.tab.curl': 'cURL',
    'examples.tab.python': 'Python',
    'examples.tab.js': 'JavaScript',
    'examples.tab.node': 'Node.js',
    'examples.tab.php': 'PHP',
    'examples.select_forwarder': 'Select a forwarder',
    'examples.no_forwarder':
      'Create a forwarder above to see paste-ready examples.',
    'examples.select_credential': 'Select a credential',
    'examples.no_credential':
      'Create a credential above to see paste-ready examples.',

    // Health check
    'health.check': 'Health check',
    'health.checking': 'Checking…',
    'health.ok': 'All systems ok',
    'health.failed': 'Health check failed',
    'health.usage_label': 'Your usage',
    'health.calls_all_time': 'total calls',
    'health.bytes_transferred': 'bytes proxied',
    'health.credentials_active': 'active credentials',
    'health.forwarders_active': 'active forwarders',
    'health.plan': 'plan',
    'health.no_plan': 'no plan assigned',
    'health.last_checked': 'last checked',
    'nodes.title': 'My Nodes',
    'nodes.add': 'Add Node',
    'nodes.kicker': 'Network contribution',
    'nodes.hero_title': 'Add your own server to ProxyNet',
    'nodes.hero_desc':
      'Connect a server you control and help expand the network. You will receive a one-time installation command after creating the node.',
    'nodes.view_guide': 'View setup guide',
    'nodes.get_started': 'Getting started',
    'nodes.how_title': 'Three steps to connect a node',
    'nodes.secure_setup': 'Secure enrollment',
    'nodes.step_one_title': 'Describe the server',
    'nodes.step_one_desc':
      'Choose a recognizable label and the server’s country.',
    'nodes.step_two_title': 'Run the installer',
    'nodes.step_two_desc':
      'Copy the one-time command and run it as root on your Linux server.',
    'nodes.step_three_title': 'Wait for approval',
    'nodes.step_three_desc':
      'The agent connects securely. An admin approves it before traffic is accepted.',
    'nodes.next_step': 'Next step',
    'nodes.install_title': 'Install the node agent',
    'nodes.install_hint':
      'Run this command on the server you want to connect. Keep the token private and use it only once.',
    'nodes.install_step1': 'Open a root shell on your server.',
    'nodes.install_step2': 'Paste and run the command below.',
    'nodes.install_step3': 'Return here and wait for approval.',
    'nodes.expires': 'Token expires:',
    'nodes.approval_hint': 'Admin approval is required before activation.',
    'nodes.inventory': 'Your infrastructure',
    'nodes.your_nodes': 'Connected nodes',
    'nodes.refresh': 'Refresh',
    'nodes.total': 'Total nodes',
    'nodes.healthy_count': 'Healthy',
    'nodes.pending_count': 'Needs setup / approval',
    'nodes.created': 'Created',
    'nodes.agent': 'Agent',
    'nodes.add_intro_title': 'Register a server',
    'nodes.add_intro':
      'Give your server a label and choose its location. We will generate a secure enrollment command next.',
    'nodes.required': 'Required',
    'nodes.country_select': 'Select a country',
    'nodes.country_help':
      'Use the country where the server is physically hosted.',
    'nodes.no_countries':
      'Country data is temporarily unavailable. Please try again in a moment.',
    'nodes.empty': 'No nodes yet',
    'nodes.empty_msg': 'Contribute a node to the network and earn trust.',
    'nodes.label': 'Node label',
    'nodes.label.placeholder': 'e.g. frankfurt-vps-01',
    'nodes.country': 'Country code (ISO 2)',
    'nodes.country.placeholder': 'e.g. DE',
    'nodes.create': 'Add Node',
    'nodes.creating': 'Adding…',
    'nodes.status.pending': 'Pending approval',
    'nodes.status.active': 'Active',
    'nodes.status.healthy': 'Healthy',
    'nodes.status.degraded': 'Degraded',
    'nodes.status.unhealthy': 'Unhealthy',
    'nodes.status.quarantined': 'Quarantined',
    'nodes.status.draining': 'Draining',
    'nodes.status.disabled': 'Rejected',
    'nodes.enrolled': 'Agent connected',
    'nodes.not_enrolled': 'Waiting for agent',
    'nodes.token_title': 'Enrollment token',
    'nodes.token_hint':
      'Run this command on your server to connect the agent. Token expires in 7 days.',
    'nodes.copy_cmd': 'Copy command',
    'nodes.copied': 'Copied!',
    'nodes.dismiss': 'Dismiss',
    // Admin nodes (pending approval)
    'admin.nodes.pending': 'Pending Approval',
    'admin.nodes.submitted_by': 'Submitted by',
    'admin.nodes.approve': 'Approve',
    'admin.nodes.reject': 'Reject',
    'admin.nodes.all': 'All Nodes',

    // User profile
    'prof.title': 'Profile',
    'prof.account': 'Account',
    'prof.name': 'Display name',
    'prof.email': 'Email',
    'prof.routing': 'Routing Settings',
    'prof.mode': 'Mode',
    'prof.auto': 'Auto',
    'prof.auto_desc':
      'Use an available healthy or active relay selected automatically by the platform.',
    'prof.country': 'Specific Country',
    'prof.country_desc': 'Keep requests on a relay in the country you choose.',
    'prof.country_label': 'Country code (e.g. DE, US)',
    'prof.country_search': 'Search countries…',
    'prof.country_help':
      'Only enabled countries are shown. A route is used only when a healthy relay is available there.',
    'prof.country_nodes': '{healthy} healthy · {total} total',
    'prof.country_unavailable': 'No available relay',
    'prof.country_none': 'No matching countries',
    'prof.country_selected': 'Selected: {country}',
    'prof.save': 'Save settings',
    'prof.saving': 'Saving…',
    'prof.saved': 'Saved!',
  },

  fa: {
    brand: 'پروکسی‌نت',
    signin: 'ورود',
    signout: 'خروج',
    register: 'ثبت‌نام',
    save: 'ذخیره',
    cancel: 'لغو',
    loading: 'در حال بارگذاری...',
    back_home: 'خانه',

    'lang.en': 'EN',
    'lang.fa': 'فا',

    'nav.signin': 'ورود',
    'nav.getstarted': 'شروع کنید',

    'hero.badge': 'شبکه آنلاین · ۱ نود فعال',
    'hero.title': 'شبکه‌ای امن و پایدار برای برنامه‌های مدرن',
    'hero.subtitle':
      'اتصال امن، مسیریابی انعطاف‌پذیر از طریق زیرساخت مدیریت‌شده و کنترل شفاف مصرف برای افراد و تیم‌ها.',
    'hero.cta_primary': 'ایجاد حساب رایگان',
    'hero.cta_secondary': 'ورود',

    'features.title': 'همه چیزی که نیاز دارید',
    'features.subtitle':
      'طراحی‌شده برای حفظ حریم خصوصی، عملکرد، پایداری و مدیریت شفاف شبکه.',
    'feat.0.title': 'اتصال‌های امن',
    'feat.0.desc':
      'رمزنگاری قدرتمند و طراحی مبتنی بر حریم خصوصی برای اتصال‌های پایدار.',
    'feat.1.title': 'نودهای خروجی جهانی',
    'feat.1.desc':
      'از طریق نودهای خروجی تأیید شده در کشورهای مختلف مسیریابی کنید. کشور خاصی انتخاب کنید یا بگذارید پلتفرم بهترین مسیر را انتخاب کند.',
    'feat.2.title': 'ارسال بدون سربار',
    'feat.2.desc':
      'مسیریابی کارآمد، عملکردی سریع و پاسخ‌گو را در کنار اتصال‌های محافظت‌شده فراهم می‌کند.',
    'feat.3.title': 'اتصال پایدار',
    'feat.3.desc':
      'مسیریابی آگاه از سلامت نودها و نقاط پایانی سازگار، پایداری سرویس را با تغییر شرایط شبکه حفظ می‌کند.',
    'feat.4.title': 'نظارت بر مصرف',
    'feat.4.desc':
      'شمارنده‌های پهنای باند و اتصال بلادرنگ به ازای هر اعتبارنامه. دقیقاً بدانید تخصیص شما چگونه استفاده می‌شود.',
    'feat.5.title': 'اعتبارنامه‌های متعدد',
    'feat.5.desc':
      'اعتبارنامه‌های پراکسی جداگانه برای دستگاه‌های مختلف ایجاد کنید. یک اعتبارنامه را بدون تأثیر بر بقیه لغو کنید.',

    'steps.title': 'در چند دقیقه آماده به کار',
    'steps.subtitle': 'بدون دردسر تنظیمات. سه مرحله و متصل می‌شوید.',
    'step.1.title': 'ایجاد حساب',
    'step.1.desc': 'با ایمیل یا موبایل خود ثبت‌نام کنید.',
    'step.2.title': 'ایجاد اعتبارنامه',
    'step.2.desc': 'یک اعتبارنامه اتصال از داشبورد خود ایجاد کنید.',
    'step.3.title': 'اتصال',
    'step.3.desc': 'جزئیات اتصال را در کلاینت سازگار موردنظر خود استفاده کنید.',

    'cta.title': 'آماده اتصال هستید؟',
    'cta.subtitle':
      'حساب رایگان خود را ایجاد کنید و اولین اعتبارنامه را در کمتر از ۲ دقیقه بسازید.',
    'cta.btn': 'شروع رایگان',

    'footer.rights': '© ۲۰۲۶ پروکسی‌نت. تمام حقوق محفوظ است.',
    'footer.signin': 'ورود',
    'footer.register': 'ثبت‌نام',

    'login.title': 'پروکسی‌نت',
    'login.subtitle': 'برای ادامه وارد شوید',
    'login.tab.password': 'رمز عبور',
    'login.tab.sms': 'پیامک',
    'login.email': 'ایمیل',
    'login.email.placeholder': 'شما@مثال.com',
    'login.password': 'رمز عبور',
    'login.password.placeholder': '••••••••',
    'login.phone': 'شماره موبایل',
    'login.phone.placeholder': '۰۹xxxxxxxxx',
    'login.sendcode': 'ارسال کد',
    'login.sending': 'در حال ارسال...',
    'login.code': 'کد تأیید',
    'login.code.placeholder': 'کد ۶ رقمی',
    'login.code_sent': 'کد ارسال شد! کد ۶ رقمی را وارد کنید.',
    'login.submit': 'ورود',
    'login.signing': 'در حال ورود...',
    'login.verify': 'تأیید و ورود',
    'login.no_account': 'حساب ندارید؟',
    'login.home': 'خانه',
    'login.change_phone': 'تغییر شماره',

    'reg.title': 'ایجاد حساب',
    'reg.subtitle': 'در ثانیه‌ها به پروکسی‌نت بپیوندید',
    'reg.name': 'نام نمایشی',
    'reg.name.placeholder': 'نام شما',
    'reg.email': 'ایمیل',
    'reg.email.placeholder': 'شما@مثال.com',
    'reg.phone': 'شماره موبایل',
    'reg.phone.placeholder': '۰۹xxxxxxxxx',
    'reg.phone.hint': 'برای تأیید پیامکی و ورود الزامی است',
    'reg.password': 'رمز عبور',
    'reg.password.placeholder': 'حداقل ۶ کاراکتر',
    'reg.confirm': 'تکرار رمز عبور',
    'reg.confirm.placeholder': 'رمز عبور را تکرار کنید',
    'reg.submit': 'ایجاد حساب',
    'reg.creating': 'در حال ایجاد حساب...',
    'reg.have_account': 'حساب دارید؟',
    'reg.err.mismatch': 'رمزهای عبور مطابقت ندارند',
    'reg.err.short': 'رمز عبور باید حداقل ۶ کاراکتر باشد',

    'admin.panel': 'پنل ادمین',
    'admin.nav.dashboard': 'داشبورد',
    'admin.nav.statistics': 'آمار و بودجه',
    'admin.nav.nodes': 'نودها',
    'admin.nav.users': 'کاربران',
    'admin.nav.plans': 'پلن‌ها',
    'admin.nav.pricing': 'تعرفه‌ها',
    'admin.nav.countries': 'کشورها',
    'admin.signout': 'خروج',

    'user.panel': 'پروکسی‌نت',
    'user.nav.dashboard': 'داشبورد',
    'user.nav.credentials': 'اعتبارنامه‌های من',
    'user.nav.profile': 'پروفایل',
    'user.signout': 'خروج',

    'udash.welcome': 'خوش آمدید',
    'udash.bandwidth': 'پهنای باند مصرفی',
    'udash.connections': 'اتصالات',
    'udash.credentials': 'اعتبارنامه‌های فعال',
    'udash.monthly': 'پهنای باند ماهانه',
    'udash.plan': 'پلن فعلی',
    'udash.of': 'از',
    'udash.used': 'استفاده شده',
    'udash.period': 'دوره',
    'udash.no_plan': 'پلن فعالی ندارید',
    'udash.no_plan_msg': 'برای دریافت پلن با ادمین تماس بگیرید.',
    'udash.proxy_title': 'اعتبارنامه‌های پراکسی',
    'udash.proxy_msg':
      'اعتبارنامه‌های VLESS خود را مدیریت کنید و آدرس اتصال را کپی کنید.',
    'udash.manage': 'مدیریت اعتبارنامه‌ها',
    'udash.account_title': 'تنظیمات حساب',
    'udash.account_msg': 'نام نمایشی و تنظیمات مسیریابی خود را به‌روز کنید.',
    'udash.edit_profile': 'ویرایش پروفایل',

    'cred.title': 'اعتبارنامه‌های من',
    'cred.new': 'اعتبارنامه جدید',
    'cred.create': 'ایجاد',
    'cred.revoke': 'لغو',
    'cred.copy': 'کپی آدرس',
    'cred.copied': 'کپی شد!',
    'cred.empty': 'هنوز اعتبارنامه‌ای ندارید',
    'cred.empty_msg': 'اولین اعتبارنامه پراکسی را برای شروع اتصال ایجاد کنید.',
    'cred.label.placeholder': 'برچسب (اختیاری)',
    'cred.created': 'ایجاد شده',
    'cred.status.active': 'فعال',
    'cred.status.revoked': 'لغو شده',

    // User nodes (FA)
    'user.nav.nodes': 'نودهای من',
    'user.nav.forwarders': 'فوروارد آدرس‌ها',
    'user.nav.wallet': 'کیف پول',
    'user.nav.billing': 'گزارش مصرف و مالی',

    // Wallet page (FA)
    'wallet.title': 'کیف پول',
    'wallet.balance': 'موجودی',
    'wallet.currency': 'تومان',
    'wallet.low_balance_warning':
      'موجودی شما کمتر از حداقل مورد نیاز برای هر فراخوانی است. برای ادامه استفاده از API، شارژ کنید.',
    'wallet.zero_balance':
      'موجودی کیف پول شما صفر است. هر درخواست حداقل به {min} تومان نیاز دارد.',
    'wallet.pricing_title': 'تعرفه فعلی',
    'wallet.pricing_per_request': 'به ازای هر درخواست',
    'wallet.pricing_per_mb': 'به ازای هر مگابایت انتقالی',
    'wallet.pricing_min_balance': 'حداقل موجودی برای فراخوانی',
    'wallet.transactions': 'تراکنش‌های اخیر',
    'wallet.no_transactions': 'هنوز تراکنشی ثبت نشده است.',
    'wallet.tx.date': 'تاریخ',
    'wallet.tx.type': 'نوع',
    'wallet.tx.description': 'توضیح',
    'wallet.tx.amount': 'مبلغ',
    'wallet.tx.balance': 'موجودی',
    'wallet.topup_note':
      'برای افزایش موجودی با پشتیبانی تماس بگیرید. اتصال به درگاه پرداخت به‌زودی اضافه می‌شود.',
    'wallet.type.topup_admin': 'شارژ توسط مدیر',
    'wallet.type.forwarder_call': 'فراخوانی فورواردر',
    'wallet.type.gateway_call': 'فراخوانی گیت‌وی',
    'wallet.type.adjustment_admin': 'تنظیم توسط مدیر',
    'wallet.type.refund': 'بازگشت',

    // Forwarders page (FA)
    'fwd.title': 'فوروارد آدرس‌ها',
    'fwd.intro':
      'یک URL کوتاه بسازید که درخواست را از طریق شبکه ما به هر مقصد خارجی هدایت می‌کند. برای فراخوانی API در کد شما عالی است — URL ما را در جای URL مقصد در کد قرار دهید.',
    'fwd.new': 'فوروارد جدید',
    'fwd.empty': 'هنوز فورواردی ندارید.',
    'fwd.form.label': 'برچسب',
    'fwd.form.label_ph': 'مثال: «API آب و هوا» یا «تماس با GitHub»',
    'fwd.form.target': 'URL مقصد',
    'fwd.form.target_ph': 'https://api.example.com/data',
    'fwd.form.target_help':
      'مقصد نهایی که درخواست‌های فوروارد شده به آن ارسال می‌شود.',
    'fwd.form.forward_auth': 'ارسال هدر Authorization به مقصد',
    'fwd.form.preserve_path': 'افزودن پسوند مسیر کلاینت به URL مقصد',
    'fwd.form.preserve_query': 'ارسال کوئری استرینگ به مقصد',
    'fwd.form.create': 'ایجاد',
    'fwd.your_url': 'URL فورواردر شما',
    'fwd.your_url_help':
      'این را در کد خود به جای URL مقصد بگذارید. هر متد HTTP، هدر، بدنه، پسوند مسیر و کوئری استرینگ عبور داده می‌شود.',
    'fwd.calls': 'فراخوانی',
    'fwd.last_used': 'آخرین استفاده',
    'fwd.never_used': 'استفاده نشده',
    'fwd.enabled': 'فعال',
    'fwd.disabled': 'غیرفعال',
    'fwd.disable': 'غیرفعال کن',
    'fwd.enable': 'فعال کن',
    'fwd.delete': 'حذف',
    'fwd.confirm_delete': 'این فورواردر حذف شود؟ URL بلافاصله از کار می‌افتد.',
    'fwd.example': 'مثال',
    'fwd.test': 'تست',
    'fwd.testing': 'در حال تست…',
    'fwd.test_ok': 'موفق',
    'fwd.test_fail': 'ناموفق',
    'fwd.test_hint':
      'یک درخواست GET به URL شما ارسال می‌کند و به عنوان یک فراخوانی شمرده می‌شود.',

    // Usage examples panel (FA)
    'examples.title': 'نحوه استفاده',
    'examples.intro.forwarder':
      'URL فورواردر خود را در کد قرار دهید — بدون هدر Auth و بدون Wrapper. با هر متد HTTP کار می‌کند.',
    'examples.intro.gateway':
      'هر URL را با متد POST به /gateway/fetch بفرستید و UUID و Secret را به‌عنوان Basic Auth استفاده کنید. پاسخ در قالب JSON برگردانده می‌شود تا کد شما بتواند وضعیت و هدرها را بررسی کند.',
    'examples.tab.curl': 'cURL',
    'examples.tab.python': 'پایتون',
    'examples.tab.js': 'جاوااسکریپت',
    'examples.tab.node': 'Node.js',
    'examples.tab.php': 'پی‌اچ‌پی',
    'examples.select_forwarder': 'یک فورواردر انتخاب کنید',
    'examples.no_forwarder': 'برای دیدن مثال آماده، ابتدا یک فورواردر بسازید.',
    'examples.select_credential': 'یک اعتبارنامه انتخاب کنید',
    'examples.no_credential':
      'برای دیدن مثال آماده، ابتدا یک اعتبارنامه بسازید.',

    // Health check (FA)
    'health.check': 'بررسی سلامت',
    'health.checking': 'در حال بررسی…',
    'health.ok': 'همه چیز خوب کار می‌کند',
    'health.failed': 'بررسی سلامت ناموفق بود',
    'health.usage_label': 'مصرف شما',
    'health.calls_all_time': 'تعداد فراخوانی',
    'health.bytes_transferred': 'بایت منتقل‌شده',
    'health.credentials_active': 'اعتبارنامه‌های فعال',
    'health.forwarders_active': 'فورواردهای فعال',
    'health.plan': 'پلن',
    'health.no_plan': 'پلن اختصاص نیافته',
    'health.last_checked': 'آخرین بررسی',
    'nodes.title': 'نودهای من',
    'nodes.add': 'افزودن نود',
    'nodes.kicker': 'مشارکت در شبکه',
    'nodes.hero_title': 'سرور خود را به پروکسی‌نت اضافه کنید',
    'nodes.hero_desc':
      'یک سرور تحت کنترل خود را متصل کنید و به گسترش شبکه کمک کنید. پس از ایجاد نود، دستور نصب یک‌بارمصرف دریافت می‌کنید.',
    'nodes.view_guide': 'مشاهده راهنمای نصب',
    'nodes.get_started': 'شروع کار',
    'nodes.how_title': 'اتصال نود در سه مرحله',
    'nodes.secure_setup': 'ثبت‌نام امن',
    'nodes.step_one_title': 'مشخصات سرور را وارد کنید',
    'nodes.step_one_desc': 'یک برچسب قابل تشخیص و کشور سرور را انتخاب کنید.',
    'nodes.step_two_title': 'نصب‌کننده را اجرا کنید',
    'nodes.step_two_desc':
      'دستور یک‌بارمصرف را کپی کرده و با دسترسی root روی سرور لینوکسی اجرا کنید.',
    'nodes.step_three_title': 'منتظر تایید بمانید',
    'nodes.step_three_desc':
      'ایجنت به‌صورت امن متصل می‌شود و قبل از پذیرش ترافیک باید توسط ادمین تایید شود.',
    'nodes.next_step': 'مرحله بعد',
    'nodes.install_title': 'نصب ایجنت نود',
    'nodes.install_hint':
      'این دستور را روی سروری که می‌خواهید متصل کنید اجرا کنید. توکن را محرمانه نگه دارید و فقط یک‌بار استفاده کنید.',
    'nodes.install_step1': 'یک ترمینال root روی سرور باز کنید.',
    'nodes.install_step2': 'دستور زیر را کپی و اجرا کنید.',
    'nodes.install_step3': 'به این صفحه برگردید و منتظر تایید بمانید.',
    'nodes.expires': 'انقضای توکن:',
    'nodes.approval_hint': 'برای فعال‌سازی، تایید ادمین لازم است.',
    'nodes.inventory': 'زیرساخت شما',
    'nodes.your_nodes': 'نودهای متصل',
    'nodes.refresh': 'به‌روزرسانی',
    'nodes.total': 'کل نودها',
    'nodes.healthy_count': 'سالم',
    'nodes.pending_count': 'نیازمند راه‌اندازی / تایید',
    'nodes.created': 'ایجاد شده',
    'nodes.agent': 'ایجنت',
    'nodes.add_intro_title': 'ثبت یک سرور',
    'nodes.add_intro':
      'برچسب و محل سرور را وارد کنید. سپس یک دستور ثبت‌نام امن برای شما ساخته می‌شود.',
    'nodes.required': 'الزامی',
    'nodes.country_select': 'کشور را انتخاب کنید',
    'nodes.country_help':
      'کشوری را انتخاب کنید که سرور به‌صورت فیزیکی در آن میزبانی می‌شود.',
    'nodes.no_countries':
      'اطلاعات کشورها موقتاً در دسترس نیست. چند لحظه دیگر دوباره تلاش کنید.',
    'nodes.empty': 'هنوز نودی ندارید',
    'nodes.empty_msg': 'یک نود به شبکه اضافه کنید و اعتماد کسب کنید.',
    'nodes.label': 'برچسب نود',
    'nodes.label.placeholder': 'مثلاً frankfurt-vps-01',
    'nodes.country': 'کد کشور (ISO 2)',
    'nodes.country.placeholder': 'مثلاً DE',
    'nodes.create': 'افزودن نود',
    'nodes.creating': 'در حال افزودن...',
    'nodes.status.pending': 'در انتظار تایید',
    'nodes.status.active': 'فعال',
    'nodes.status.healthy': 'سالم',
    'nodes.status.degraded': 'ضعیف',
    'nodes.status.unhealthy': 'ناسالم',
    'nodes.status.quarantined': 'قرنطینه',
    'nodes.status.draining': 'در حال تخلیه',
    'nodes.status.disabled': 'رد شده',
    'nodes.enrolled': 'ایجنت متصل است',
    'nodes.not_enrolled': 'منتظر ایجنت',
    'nodes.token_title': 'توکن ثبت‌نام',
    'nodes.token_hint':
      'این دستور را روی سرور خود اجرا کنید تا ایجنت متصل شود. توکن ۷ روز اعتبار دارد.',
    'nodes.copy_cmd': 'کپی دستور',
    'nodes.copied': 'کپی شد!',
    'nodes.dismiss': 'بستن',
    'admin.nodes.pending': 'در انتظار تایید',
    'admin.nodes.submitted_by': 'ارسال‌کننده',
    'admin.nodes.approve': 'تایید',
    'admin.nodes.reject': 'رد',
    'admin.nodes.all': 'همه نودها',

    'prof.title': 'پروفایل',
    'prof.account': 'حساب',
    'prof.name': 'نام نمایشی',
    'prof.email': 'ایمیل',
    'prof.routing': 'تنظیمات مسیریابی',
    'prof.mode': 'حالت',
    'prof.auto': 'خودکار',
    'prof.auto_desc':
      'یک رله سالم یا فعال را به‌صورت خودکار توسط سامانه انتخاب کنید.',
    'prof.country': 'کشور خاص',
    'prof.country_desc':
      'درخواست‌ها را از یک رله در کشور انتخابی شما عبور می‌دهد.',
    'prof.country_label': 'کد کشور (مثلاً DE، US)',
    'prof.country_search': 'جستجوی کشورها...',
    'prof.country_help':
      'فقط کشورهای فعال نمایش داده می‌شوند. مسیر فقط زمانی استفاده می‌شود که رله سالمی در آن کشور در دسترس باشد.',
    'prof.country_nodes': '{healthy} سالم · {total} کل',
    'prof.country_unavailable': 'رله‌ای در دسترس نیست',
    'prof.country_none': 'کشور منطبقی پیدا نشد',
    'prof.country_selected': 'انتخاب‌شده: {country}',
    'prof.save': 'ذخیره تنظیمات',
    'prof.saving': 'در حال ذخیره...',
    'prof.saved': 'ذخیره شد!',
  },
};

export function t(lang: Lang, key: string): string {
  return tr[lang]?.[key] ?? tr.en[key] ?? key;
}

export default tr;
